...
from basic_pitch import ICASSP_2022_MODEL_PATH, build_icassp_2022_model_path, FilenameSuffix

# Prefer ONNX backend (works with TF 2.21+ where the TF saved model API changed)
try:
    from basic_pitch import ONNX_PRESENT
    _MODEL_PATH = build_icassp_2022_model_path(FilenameSuffix.onnx) if ONNX_PRESENT else ICASSP_2022_MODEL_PATH
except Exception:
...
def analyze_pitch(audio_path):
    ...
    try:
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            # Run Basic Pitch (uses ONNX model when available for TF 2.21+ compat)
